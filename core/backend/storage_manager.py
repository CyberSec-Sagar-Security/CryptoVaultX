...
class StorageManager:
    ...
    def create_user_folders(self, user_id: int) -> Tuple[Path, Path]:
        """
        Create storage folders for a new user
        Returns: (uploads_folder, deleted_folder)
        """
        user_folder = self.storage_root / str(user_id)
        ...
        print(f"✅ Created storage folders for user {user_id}")
        print(f"   📁 Uploads: {uploads_folder}")
        print(f"   🗑️ Deleted: {deleted_folder}")
        
        return uploads_folder, deleted_folder
    
    def get_user_folders(self, user_id: int) -> Tuple[Path, Path]:
        """Get paths to user's storage folders"""
        user_folder = self.storage_root / str(user_id)
        uploads_folder = user_folder / "uploads"
        deleted_folder = user_folder / "deleted"
        return uploads_folder, deleted_folder
    
    def generate_storage_path(self, user_id: int, original_filename: str) -> str:
        """
        ...
        uploads_folder, _ = self.get_user_folders(user_id)
    ...
    def move_to_deleted(self, storage_path: str, user_id: int) -> Optional[str]:
        """
        ...
        try:
            ...
            _, deleted_folder = self.get_user_folders(user_id)
    ...
    def get_user_storage_usage(self, user_id: int) -> dict:
        ...
        """
        uploads_folder, deleted_folder = self.get_user_folders(user_id)
    ...
    def check_quota_before_upload(self, user_id: int, file_size: int) -> Tuple[bool, str]:
        """
    ...
    def cleanup_empty_folders(self, user_id: int):
