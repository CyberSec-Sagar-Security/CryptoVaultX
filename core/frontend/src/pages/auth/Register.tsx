...
export default function Register() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const payload = { username: form.name.trim(), email: form.email.trim().toLowerCase(), password: form.password }
                ...
                <Button 
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const errorMessage = err instanceof Error ? err.message : 'Direct API call failed'
                      setMessage({ type: 'error', text: `Direct API failed: ${errorMessage}` })
                    } finally { 
                      setLoading(false) 
                    }
                  }}
