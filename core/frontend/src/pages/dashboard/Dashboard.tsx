...
const Dashboard: React.FC = () => {
  ...
  // Fetch dashboard data from local storage and API
  const fetchDashboardData = async () => {
    try {
      ...
      // Try to get files from API as additional source
      let apiFiles: any[] = [];
      try {
      ...
      // Combine local and API files
      const allFiles = [
        ...localFiles,
        ...apiFiles.filter(apiFile => !localFiles.some(localFile => localFile.filename === apiFile.filename))
      ];
      
      // Calculate stats from combined files
      const totalFiles = allFiles.length;
      const encryptedFiles = allFiles.filter((f: any) => f.is_encrypted).length;
      const totalBytes = allFiles.reduce((sum: number, f: any) => sum + (f.size || 0), 0);
      ...
      setStats({
        ...
        sharedFiles: 0 // TODO: Implement sharing count when sharing is added
      });

      // Set recent files (limit to 4 most recent)
      const sortedFiles = allFiles
        ...
        .map((file: any) => ({
          ...
          status: file.is_encrypted ? ENCRYPTION_STATUS.ENCRYPTED : ENCRYPTION_STATUS.NONE,
          lastModified: getRelativeTime(file.created_at),
          sharedWith: 0 // TODO: Implement when sharing is added
        }));
  ...
  // Listen for file upload events to refresh the dashboard
  useEffect(() => {
    const handleFileUploaded = () => {
      fetchDashboardData(); // Refresh dashboard data when a new file is uploaded
    };
        ...
        >
          <div className="flex justify-between items-center mb-6">
            ...
            <Button 
              ...
              size="sm"
              onClick={() => navigate('/analytics')}
              className="border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/10 hover:border-cyan-400 shadow-lg"
            >
              View Full Analytics
