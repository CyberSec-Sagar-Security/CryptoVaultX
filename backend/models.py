...
DATABASE_URL = os.getenv('DATABASE_URL', 'postgresql://admin:admin123@db:5432/cryptovault_db')
...
class User(Base):
    ...
    # Relationships
    uploaded_files = relationship("File", back_populates="owner", cascade="all, delete-orphan")
    shared_files_received = relationship("SharedFile", foreign_keys="SharedFile.shared_with_user_id", back_populates="shared_with_user", cascade="all, delete-orphan")
    shared_files_given = relationship("SharedFile", foreign_keys="SharedFile.shared_by_user_id", back_populates="shared_by_user", cascade="all, delete-orphan")
...
class File(Base):
    ...
    # Encryption metadata
    encryption_key_hash = Column(String(64), nullable=False)  # Hash of the encryption key
    ...
    shared_files = relationship("SharedFile", back_populates="file", cascade="all, delete-orphan")
...
class SharedFile(Base):
    ...
    access_count = Column(Integer, default=0, nullable=False)  # Number of times accessed
    ...
    shared_with_user = relationship("User", foreign_keys=[shared_with_user_id], back_populates="shared_files_received")
    shared_by_user = relationship("User", foreign_keys=[shared_by_user_id], back_populates="shared_files_given")
...
__all__ = [
    'Base', 'User', 'File', 'SharedFile', 'Session',
    'engine', 'SessionLocal', 'get_db', 'create_tables', 'init_database'
]
