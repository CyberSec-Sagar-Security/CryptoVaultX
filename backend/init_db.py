...
def create_sample_files(db_session, users):
    ...
    for file_data in sample_files:
        ...
        if existing_file:
            print(f"File {file_data['filename']} already exists for {owner.username}, skipping...")
        ...
        new_file = File(
            ...
            owner_id=owner.id,
            created_at=datetime.utcnow(),
            expires_at=datetime.utcnow() + timedelta(days=365) if file_data['is_public'] else None
        )
...
def create_sample_shared_files(db_session, users, files):
    ...
    for share_data in sharing_data:
        ...
        existing_share = db_session.query(SharedFile).filter(
            SharedFile.file_id == file.id,
            SharedFile.shared_with_user_id == shared_with_user.id
        ).first()
        if existing_share:
            print(f"Share already exists: {file.filename} with {shared_with_user.username}, skipping...")
...
def main():
    ...
    try:
        ...
        for user in users:
            print(f"  • {user.username} ({user.email}) - {'Verified' if user.is_verified else 'Unverified'}")
        
        print("\nSample Files Created:")
        for file in files:
            print(f"  • {file.filename} by {file.owner.username} - {'Public' if file.is_public else 'Private'}")
