...
interface UploadSettings {
  defaultEncryption: 'LOW' | 'MEDIUM' | 'HIGH';
  autoEncrypt: boolean;
...
const Upload: React.FC = () => {
  ...
  const handleFileSelect = useCallback((selectedFiles: FileList) => {
    ...
    if (invalidFiles.length > 0) {
      ...
      console.warn(`${invalidFiles.length} files exceed size limit`);
    }
  ...
  const performUpload = async (fileUpload: FileUpload) => {
    ...
    try {
      // Get user's private key for key wrapping
      ...
      if (fileUpload.autoEncrypt) {
        ...
        updateFile({ progress: 20 });
        
        // Encrypt the file
        const encryptedFile = await encryptFile(fileUpload.file, fileKey);
        ...
        // Import public key and wrap file key
        const { importPublicKeyPem } = await import('../../services/crypto');
        const publicKey = await importPublicKeyPem(publicKeyPem);
        const wrappedKey = await wrapKey(fileKey, publicKey);
        updateFile({ progress: 50 });

        // Upload phase
        updateFile({ status: 'uploading', progress: 60 });
        ...
        const encryptedBlob = new Blob([
          new Uint8Array(12), // IV placeholder
          new TextEncoder().encode(encryptedFile.ciphertext)
        ], { type: 'application/octet-stream' });
        
        formData.append('file', encryptedBlob, fileUpload.file.name);
        formData.append('originalName', fileUpload.file.name);
        formData.append('mimeType', fileUpload.file.type);
        formData.append('size', fileUpload.file.size.toString());
        ...
        // Upload without encryption (not recommended)
        updateFile({ status: 'uploading', progress: 60 });
